...
-- Technically,  instance Buildable SlotPoint
-- in an orphan instance, but `SlotPoint` is a type synonym
...
compareSlot pt1 pt2 = compare (toSlotPoint pt1) (toSlotPoint pt2)
...
--
-- In contrast to 'ChainPoint', the type 'SlotPoint' does not refer
-- to a point on an actual chain with valid block hashes,
-- but merely to a timeslot which can hold a single block.
-- This implies:
--
-- * 'SlotPoint' has a linear ordering implemented in the 'Ord' class
--   (where @Origin < At slot@).
-- * Using 'SlotPoint' in QuickCheck testing requires less context
-- (such as an actual simulated chain.)
--
-- TODO: Better name, perhaps simply 'Slot'?
type SlotPoint = WithOrigin SlotNo

-- | Retrieve the slot of a 'ChainPoint'.
toSlotPoint :: ChainPoint -> SlotPoint
toSlotPoint ChainPointAtGenesis = Origin
toSlotPoint (ChainPoint slot _) = At slot

instance Buildable SlotPoint where
