...
a 'SlotPoint' as argument, which can represent the 'Origin'.
...
the 'SlotPoint' type, or handled slightly differently when it
...
getPoint :: W.Wallet s -> W.SlotPoint
getPoint =
    W.toSlotPoint . W.chainPointFromBlockHeader . view #currentTip
...
data Checkpoints a = Checkpoints
    { checkpoints :: Map W.SlotPoint a
    } deriving (Eq,Show,Generic)
-- FIXME LATER during ADP-1043:
--  Use a more sophisticated 'Checkpoints' type that stores deltas.

singleton :: W.SlotPoint -> a -> Checkpoints a
...
selectAllCheckpoints
    :: forall s. PersistState s
    => W.WalletId
    -> SqlPersistT IO [(W.SlotPoint, W.Wallet s)]
...
-- violated.
data ErrRollbackTo = ErrNoOlderCheckpoint W.WalletId W.SlotPoint deriving (Show)
