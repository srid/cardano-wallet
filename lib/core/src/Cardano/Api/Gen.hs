...
genLovelace = Lovelace <$> frequency
    ...
    , (65, choose (1_000_000, 1_000_000_000))
    , (10, choose (1_000_000, 1_000_000_000_000_000))
    , (10, genEncodingBoundaryLovelace)
    ]
  where
    genEncodingBoundaryLovelace :: Gen Integer
    genEncodingBoundaryLovelace = do
        -- https://json.nlohmann.me/features/binary_formats/cbor/
        -- Generate a point just below a boundary
        boundary <- subtract 1 <$> elements [ 24, 256, 65536, 4294967296 ]
        Small x <- arbitrary @(Small Int)
        oneof
            [ pure $ max 0 $ fromIntegral $ boundary + x
            , pure $ fromIntegral $ boundary `div` (max 1 x)
            ]
...
genPolicyId = frequency
      -- mostly from a small number of choices, so we get plenty of repetition
    [ (3, pure $ fromString ('a' : replicate 55 '0'))
    , (8, elements [ fromString (x : replicate 55 '0') | x <- ['a'..'c'] ])
...
genValueForTxOut = do
    assets <- genAssetValueForTxOut

    -- Have a very low probability to generate a value which doesn't even
    -- specify an ada value, in case it is different from specifying a 0 ada
    -- value.
    frequency
      [ (1, pure assets)
      , (99, (assets <>) <$> genAdaValue)
      ]
  where
    genAdaValue = oneof
        [ valueFromList . (:[]) . (AdaAssetId,) . fromIntegral . unLovelace
            <$> scale (`div` 4) genLovelace
        , pure mempty
        ]
...
-- | Generate a 'Value' suitable for usage in a transaction output, but with no
-- ada.
genAssetValueForTxOut :: Gen Value
genAssetValueForTxOut  =
  valueFromList <$> listOf ((,) <$> genAssetIdNoAda <*> genUnsignedQuantity)
